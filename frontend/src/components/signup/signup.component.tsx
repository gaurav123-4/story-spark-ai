...
const SignUpComponent = () => {
  ...
  const handleResendOtp = async () => {
    ...
    setIsBusy(true);
    try {
      const otpPayload = { name: registerInfo.name, email: registerInfo.email };
      const res = await emailVerify({ ...otpPayload }).unwrap();
      if (res?.data) {
        const { expiresAt } = res.data;
        setExpiredAt(new Date(expiresAt).getTime());
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-slate-50 dark:bg-slate-950 px-4 py-12 relative overflow-hidden text-slate-900 dark:text-slate-100">
      {/* Background glows */}
      <motion.div
      ...
      >
        ...
        <div className="mb-8 text-center">
          <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-indigo-400">
            STORY SPARK AI
          </h1>
        </div>

        {/* Sign Up card */}
        <div className="w-full bg-white dark:bg-slate-800/60 backdrop-blur-xl border border-slate-200 dark:border-slate-700/50 rounded-2xl p-6 sm:p-8 shadow-2xl">
          {/* Back to Home */}
          <button
            onClick={() => (window.location.href = "/")}
            className="mb-5 text-sm text-blue-500 dark:text-blue-400 hover:text-blue-600 dark:hover:text-blue-300 transition-colors duration-200 flex items-center gap-1.5 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 rounded"
          >
            ← Back to Home
          </button>
          ...
          {!showOtpField && (
            <div className="relative mb-6">
              ...
              <div className="relative flex justify-center">
                <span className="bg-white dark:bg-slate-800 px-4 text-xs font-semibold tracking-wider uppercase text-slate-500">
                  Sign up with email
                </span>
              </div>
            </div>
                  ...
                  <ul className="space-y-1.5 list-none p-0 m-0 text-[11px] font-medium">
                    {PASSWORD_REQUIREMENTS.map(({ key, label }) => {
                      ...
                      return (
                        ...
                        >
                          <i
                            className={`fa-solid ${met ? "fa-circle-check" : "fa-circle-xmark"} text-xs shrink-0`}
                            aria-hidden="true"
                          />
                          <span>{label}</span>
                        </li>
            ...
            <div className="space-y-4 w-full">
              ...
              <div className="text-center pt-1">
                <button
                  ...
                  className="text-xs font-bold uppercase tracking-wider text-blue-600 dark:text-blue-400 hover:text-blue-500 dark:hover:text-blue-300 disabled:text-slate-400 dark:disabled:text-slate-600 transition-colors duration-150 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 rounded disabled:cursor-not-allowed cursor-pointer"
                >
          ...
          {!showOtpField && (
            <>
              ...
              <p className="mt-6 text-center text-sm text-slate-500 dark:text-slate-400">
                ...
                <Link
                  to="/login"
                  className="font-semibold text-blue-600 dark:text-blue-400 hover:underline focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 rounded"
                >
                  Sign In
                </Link>
              </p>
